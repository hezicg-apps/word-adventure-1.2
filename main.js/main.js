...
// פענוח הנתונים מה-URL (תומך בעברית UTF-8)
function loadFromUrl() {
    ...
    if (data) {
        try {
            ...
            const title = lines[0] || "Word Adventure";
            words = lines.slice(1).filter(line => line.includes('-')).map(line => {
                const [eng, heb] = line.split('-').map(s => s.trim());
                return { eng, heb };
            });
            if (words.length > 0) renderApp(title);
            else throw new Error("No words found");
        } catch (e) {
            document.getElementById('app').innerHTML = `<div class="text-center p-10 bg-white rounded-3xl shadow-lg border-2 border-red-200">
                <h2 class="text-2xl font-bold text-red-500 mb-2">אופס! שגיאה בטעינה</h2>
                <p>לא הצלחנו למצוא את המילים בקישור הזה.</p>
            </div>`;
        }
    }
}
    ...
    app.innerHTML = `
        <div class="text-center mb-8 animate-fade-in w-full">
            ...
            <div class="flex gap-3 justify-center">
                <button onclick="switchMode('cards')" id="btn-cards" class="px-6 py-2 bg-blue-600 text-white rounded-2xl font-bold shadow-lg transition-all">כרטיסיות 🗂️</button>
                <button onclick="switchMode('quiz')" id="btn-quiz" class="px-6 py-2 bg-white border-2 border-green-500 text-green-600 rounded-2xl font-bold hover:bg-green-50 transition-all">מבחן 📝</button>
            </div>
        </div>
...
function switchMode(mode) {
    ...
    document.getElementById('btn-cards').className = isCards ? "px-6 py-2 bg-blue-600 text-white rounded-2xl font-bold shadow-lg" : "px-6 py-2 bg-white border-2 border-blue-500 text-blue-600 rounded-2xl font-bold";
    document.getElementById('btn-quiz').className = !isCards ? "px-6 py-2 bg-green-600 text-white rounded-2xl font-bold shadow-lg" : "px-6 py-2 bg-white border-2 border-green-500 text-green-600 rounded-2xl font-bold";
...
function renderCards() {
    ...
    container.innerHTML = `
        ...
        <div class="flex gap-10 mt-10 items-center">
            <button onclick="changeCard(-1)" class="text-5xl hover:scale-125 transition-transform">➡️</button>
    ...
    if (currentIndex >= words.length) {
        document.getElementById('game-container').innerHTML = `
            <div class="text-center p-10 bg-white rounded-[3rem] shadow-2xl animate-fade-in border-t-8 border-green-500">
                ...
                <p class="text-xl mb-6">סיימתם את המבחן על כל המילים.</p>
                <button onclick="switchMode('cards')" class="px-8 py-3 bg-blue-600 text-white rounded-2xl font-bold text-lg">תרגול חוזר</button>
            </div>`;
    ...
    document.getElementById('game-container').innerHTML = `
        <div class="w-full max-w-md bg-white p-8 rounded-[2.5rem] shadow-2xl border-b-8 border-green-500 animate-fade-in">
            ...
            <div class="grid gap-3">
                ${options.map(opt => `<button onclick="checkAnswer('${opt}', '${word.heb}')" class="p-5 text-2xl font-bold border-2 border-slate-100 rounded-2xl hover:bg-green-50 hover:border-green-500 transition-all">${opt}</button>`).join('')}
            </div>
        </div>
    `;
}
...
function checkAnswer(sel, cor) {
    if (sel === cor) { currentIndex++; startQuiz(); }
    else { 
